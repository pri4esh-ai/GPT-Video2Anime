...
class ModelManager(private val context: Context) {
    companion object {
        ...
        // Set these after choosing the verified ONNX anime model.
    ...
    fun isModelInstalled(): Boolean {
        ...
        if (EXPECTED_SHA256.isBlank()) {
            return true
        }
        return sha256(modelFile).equals(
            EXPECTED_SHA256,
            ignoreCase = true
        )
    }
    suspend fun ensureModel(
        ...
        require(MODEL_URL.isNotBlank()) {
            "Anime ONNX model URL is not configured."
        }
        ...
        if (!isModelInstalled()) {
            modelFile.delete()
            throw IOException(
                "Downloaded anime model failed SHA-256 verification."
            )
        }
        modelFile
    }
    private fun downloadModel(
        ...
        val connection =
            (URL(MODEL_URL).openConnection() as HttpURLConnection).apply {
                ...
                requestMethod =
                    if (existingBytes > 0L) "GET" else "GET"
                if (existingBytes > 0L) {
                    setRequestProperty(
                        "Range",
                        "bytes=$existingBytes-"
        ...
        try {
            ...
            if (!append && temporaryFile.exists()) {
                temporaryFile.delete()
            }
            val totalLength =
                connection.contentLengthLong.let { length ->
                    if (length > 0L) {
                        startingBytes + length
                    } else {
                        -1L
                    }
                }
